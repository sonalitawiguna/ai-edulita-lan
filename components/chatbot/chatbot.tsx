...
export function ChatBot({ userRole }: ChatBotProps) {
  ...
  const checkApiKeyStatus = async () => {
    try {
      console.log("🔍 Checking API key status via server endpoint...")

      // Call server endpoint to check API key
      ...
      if (result.available) {
        ...
        console.log("⚠️ Google AI API key is missing or invalid:", result.error)
  ...
  const generateAIResponse = async (prompt: string): Promise<string> => {
    try {
      console.log("🤖 Generating AI response via server endpoint...")
